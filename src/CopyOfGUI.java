...
public class CopyOfGUI extends JPanel
{
	private static final File highScoreFile = new File("high.txt");
	private static final int length = 30;
	private static final int snakeSize = 15;
	
	private static int score = 0;
	private static int currentLength = 1;
	private static boolean gameEnd = false;
	private static int prevHigh;
	private static boolean newHigh = false;
	
	private static Rectangle snake[] = new Rectangle[length * length];
	private static Rectangle2D head = new Rectangle2D.Double(60, 60, snakeSize, snakeSize);
	private static Ellipse2D food = new Ellipse2D.Double((Math.random()*29)*15, (Math.random()*29)*15, snakeSize, snakeSize);

	private static Rectangle2D readInstr = new Rectangle2D.Double(120, 210, 210, 60);
	...
	{		
		...
		welcomeText.setHorizontalAlignment(start.getWidth()/2);
		...
		CopyOfGUI panel = new CopyOfGUI();
		panel.setSize(length * snakeSize, length * snakeSize +15);
		...
		Scanner highFileIn = new Scanner(highScoreFile);
		if (highScoreFile.exists())
		{
			try {
				String highScoreStr = highFileIn.nextLine();
				prevHigh = Integer.parseInt(highScoreStr);	
			}
